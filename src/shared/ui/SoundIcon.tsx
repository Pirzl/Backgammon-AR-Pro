/**
 * Responsive sound icon based on the provided SVG (speaker + sound waves).
 * `on` toggles the rendered state: when off, a diagonal slash is overlaid.
 * Sizing follows the parent font-size (uses 1em) so it scales across
 * viewports via Tailwind text-* utilities (e.g. text-[20px]).
 */
export function SoundIcon({ on, className = '' }: { on: boolean; className?: string }) {
  return (
    <svg
      className={className}
      width="1em"
      height="1em"
      viewBox="0 0 1920 1920"
      fill="currentColor"
      role="img"
      aria-hidden="true"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        fillRule="evenodd"
        d="M1129.432 113v1694.148H903.545l-451.772-451.773V564.773L903.545 113h225.887Zm542.545 248.057C1832.017 521.097 1920 733.882 1920 960.107c0 226.226-87.983 438.898-248.023 598.938l-79.851-79.85c138.694-138.582 214.93-323.018 214.93-519.087 0-196.183-76.236-380.506-214.93-519.2ZM338.83 564.773v790.602H169.415C75.672 1355.375 0 1279.703 0 1185.96V734.187c0-93.742 75.672-169.414 169.415-169.414H338.83Zm1093.922 36.085c95.776 97.018 148.407 224.644 148.407 359.16 0 134.628-52.631 262.253-148.407 359.272l-80.303-79.174c74.656-75.897 115.767-175.4 115.767-280.099 0-104.585-41.111-204.088-115.767-279.986Z"
      />
      {!on && (
        <line
          x1="160"
          y1="160"
          x2="1760"
          y2="1760"
          stroke="currentColor"
          strokeWidth="110"
          strokeLinecap="round"
        />
      )}
    </svg>
  );
}

/**
 * Controlled sound toggle button: icon + label. Parent owns `on`/`onToggle`.
 */
export function SoundToggleButton({
  on,
  onToggle,
  className = '',
}: {
  on: boolean;
  onToggle: () => void;
  className?: string;
}) {
  return (
    <button
      type="button"
      onClick={onToggle}
      aria-pressed={on}
      aria-label={on ? 'Silenciar sonido' : 'Activar sonido'}
      title={on ? 'Silenciar sonido' : 'Activar sonido'}
      className={
        'flex flex-col items-center justify-center p-3 rounded-xl border transition-all ' +
        (on
          ? 'bg-sky-500/20 border-sky-500/50 text-sky-300 shadow-[0_0_10px_rgba(14,165,233,0.2)]'
          : 'bg-white/5 border-white/10 text-white/40 hover:bg-white/10') +
        ' ' +
        className
      }
    >
      <SoundIcon on={on} className="text-[20px] mb-1" />
      <span className="text-[10px] font-bold uppercase">Sonido {on ? 'ON' : 'OFF'}</span>
    </button>
  );
}
