...
RETURNS TABLE(top_points json, top_streaks json) AS $$
DECLARE
  v_top_points json;
  v_top_streaks json;
BEGIN
  -- Top by points (fetching from wallets table directly!)
  SELECT COALESCE(json_agg(json_build_object('name', p.username, 'points', w.saldo_actual)), '[]'::json)
  INTO v_top_points
  FROM (
      SELECT user_id, saldo_actual
      FROM public.wallets
      ORDER BY saldo_actual DESC NULLS LAST
      LIMIT 10
  ) w
  ...
  RETURN QUERY SELECT v_top_points, v_top_streaks;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
